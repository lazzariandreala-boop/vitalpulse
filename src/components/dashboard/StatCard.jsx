...
export default function StatCard({ icon: Icon, label, value, unit, trend, color = 'primary', to }) {
    ...
    <>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          ...
          {/* Se non c'è valore mostra "+" per indicare che si può aggiungere */}
          {value == null && to ? (
            <span className={cn('p-1 rounded-lg text-muted-foreground/40', cfg.icon)}>
  ...
  if (to) {
    return (
      <Link to={to} className={cn(
        'relative bg-card rounded-2xl border overflow-hidden block',
